...
export class ClientesComponent {
  ...
  public clients:cadastroCliente[] = [
    {idCliente: 1, nomeCliente: 'José', sobrenomeCliente: 'Silva'}
  ]; 
  ...
  cadastrarCliente(){
    ...
    let newClient: cadastroCliente = { //variável para novo produto
      ...
      sobrenomeCliente: this.sobrenomeCliente, //Referencia a variável descrição da linha 18
  ...
  changeFormAddCliente(idCliente:number) { //Função para mostrar ou não o form add
    ...
    if (idCliente != 0) { // Condicional de validação para se o ID for diferente de 0, entrar no modo de edição
      let produto = ClientesService.getById(idCliente) // variável Produto recebe a linha do array do produto
      //Para exibir os dados na tela
      this.nomeCliente = produto.nomeCliente //Carrega a variável nome do ngModel com o campo nome da linha trazida do array
      this.sobrenomeCliente = produto.sobrenomeCliente //Carrega a variável sobrenome do ngModel com o campo sobrenome da linha trazida do array
    }
  }
